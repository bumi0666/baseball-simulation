...
def generate_full_spec_player(p_id, team, pos, num):
    ...
    return {
        ...
        "status": {
            ...
            "maxexp":     10000,
            "level":      1,
            "roster":     "active"   # "active"(1군) | "inactive"(2군)
        },
...
def run_generator():
    teams     = ["Lions", "Tigers", "Bears", "Wizards", "Eagles",
    ...
    for team in teams:
        for pos in positions:
            num = 8 if pos == "P" else 2
            for _ in range(num):
                p_data   = generate_full_spec_player(p_id, team, pos, back_number)
                filename = f"{p_id}_{p_data['name']}.json"
